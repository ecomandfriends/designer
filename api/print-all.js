  ...
  try {
    ...
    function flagToCode(val) {
      ...
      const map = {
        ...
        'france':'fr','francia':'fr','portugal':'pt','belgium':'be','netherlands':'nl',
        'holland':'nl','poland':'pl','croatia':'hr','austria':'at','switzerland':'ch','sweden':'se',
        'norway':'no','denmark':'dk','finland':'fi','greece':'gr','ireland':'ie','czech republic':'cz',
        'romania':'ro','ukraine':'ua','russia':'ru','serbia':'rs','slovakia':'sk','hungary':'hu','bulgaria':'bg',
...
.sidebar-item{display:flex;align-items:center;gap:8px;padding:7px 12px;border-bottom:1px solid #f5f5f5;cursor:pointer;transition:background .1s}
...
.sidebar-name{font-family:'Teko',sans-serif;font-size:13px;font-weight:700;line-height:1.2}
...
@media print{.toolbar,.panel,.sidebar{display:none!important}.main{display:block}.canvas-area{padding:0}.canvas-wrap{background:none!important;border:none;padding:0}}
</style>
</head><body>
<div class="toolbar">
...
<div class="main">
  <div class="sidebar">
    <div class="sidebar-head">
      <span class="sidebar-title">Pedidos (${ordersData.length})</span>
...
const selected=new Set(DATA.map((_,i)=>i)); // all selected by default
...
function badgeClass(f){
  if(f==='fulfilled')return'badge-fulfilled';
  if(f==='partial')return'badge-partial';
  return'badge-unfulfilled';
}
...
function renderSidebar(){
  list.innerHTML=DATA.map((o,i)=>{
    ...
    return'<div class="sidebar-item'+(on?' selected':'')+'" data-i="'+i+'">'+
      '<div class="sidebar-chk'+(on?' on':'')+'" data-c="'+i+'">'+(on?'✓':'')+'</div>'+
      '<div class="sidebar-info">'+
        '<div class="sidebar-name">'+o.name+'</div>'+
        '<div class="sidebar-meta">'+
          '<span>'+new Date(o.date).toLocaleDateString()+'</span>'+
          '<span class="sidebar-badge '+badgeClass(o.fulfillment)+'">'+badgeText(o.fulfillment)+'</span>'+
        '</div>'+
      '</div>'+
...
function renderSheets(){
  ...
  document.getElementById('sel-info').textContent=sel.length+' de '+DATA.length+' seleccionados · '+sel.reduce((s,i)=>s+DATA[i].stickers.length,0)+' stickers';
  if(sel.length===0){
    area.innerHTML='<div class="empty-msg">Selecciona pedidos del panel izquierdo</div>';
    return;
  }
...
document.getElementById('selAll')?.addEventListener('click',()=>{DATA.forEach((_,i)=>selected.add(i));renderSidebar();renderSheets()});
...
document.getElementById('dlPng')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    const canvas=await m.default(area,{backgroundColor:null,scale,useCORS:true,allowTaint:false,logging:false,imageTimeout:30000});
    const final=document.createElement('canvas');
    final.width=PRINT_W_PX;final.height=Math.round(canvas.height*(PRINT_W_PX/canvas.width));
    const ctx=final.getContext('2d');ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
    ctx.drawImage(canvas,0,0,final.width,final.height);
    const a=document.createElement('a');a.download='stickers_58cm.png';a.href=final.toDataURL('image/png');a.click();
