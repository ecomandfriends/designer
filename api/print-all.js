  ...
  try {
    ...
    function flagToCode(val) {
      ...
      const map = {
        ...
        'serbia':'rs','slovakia':'sk','eslovaquia':'sk','hungary':'hu','hungría':'hu','bulgaria':'bg',
        ...
        'canada':'ca','canadá':'ca','costa rica':'cr','panama':'pa','panamá':'pa','cuba':'cu',
        'dominican republic':'do','puerto rico':'pr','honduras':'hn','guatemala':'gt','jamaica':'jm',
        ...
        'turkey':'tr','turquía':'tr','turquia':'tr','saudi arabia':'sa','arabia saudí':'sa','uae':'ae',
        ...
        'albania':'al','kosovo':'xk','north macedonia':'mk','macedonia':'mk','montenegro':'me',
        'bosnia':'ba','slovenia':'si','eslovenia':'si','latvia':'lv','letonia':'lv',
        'lithuania':'lt','lituania':'lt','estonia':'ee'
      };
    ...
    // Use flag-icons CDN (supports subdivisions like gb-eng, gb-sct, gb-wls)
    function getFlagUrl(val) {
    ...
    function renderSticker(s) {
      if (s.type === 'flag') {
        const url = getFlagUrl(s.value);
        if (url) return `<div class="s s-flag"><img src="${url}" alt=""></div>`;
        return `<div class="s s-flag-fb">${s.value.replace(/[a-zA-ZáéíóúñüÁÉÍÓÚÑÜ\s]/g, '').trim() || '🏳️'}</div>`;
      }
...
.sheet{border:1.5px solid #ccc;border-radius:5px;padding:8px;page-break-inside:avoid;break-inside:avoid;width:auto}
...
.s-txt{
  ...
  line-height:1.2;padding:6px 12px;white-space:nowrap;color:#1a1a1a;
}
.s-txt.wh{color:#fff;background:#1a1a1a}

.s-flag{width:48px;height:40px;padding:5px;display:flex;align-items:center;justify-content:center}
.s-flag img{max-width:100%;max-height:100%;object-fit:contain;display:block}

.s-flag-fb{width:48px;height:40px;display:flex;align-items:center;justify-content:center;font-size:22px}
